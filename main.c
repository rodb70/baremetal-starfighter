...
GameState gameOn() {
	...
	while (1) {
		...
		for (int i = 0; i < numEnemies; i++) {
			...
			char collision = detectCollision((enemies + i)->entity,
				player.entity);
			if (collision) {
				if ((enemies + i)->ai == 4) {
					player.health -= 30;
		...
		for (int i = 0; i < numProjectiles; i++) {
			...
			if ((projectiles + i)->team == 0) {
				for (int j = 0; j < numEnemies; j++) {
					char collision = detectCollision((enemies + j)->entity,
						(projectiles+i)->entity);
					if (collision) {
				...
				char collision = detectCollision(player.entity,
					(projectiles+i)->entity);
				if (collision) {
		...
		for (int i = 0; i < numPickups; i++) {
			...
			char collision = detectCollision((pickups + i)->entity,
				player.entity);
			if (collision) {
...
void drawRectangle(int x, int y, int width, int height, vu16 color) {
...
void removeProjectile(uProjectile *projectile) {
	drawRectangle(projectile->entity.x, projectile->entity.y, projectile->entity.width, projectile->entity.height, bgColor);
	numProjectiles -= 1;
...
void updateEntity(uEntity *entity) {
	...
	if (!entity->canExit) {
		...
		if (entity->x >= 300 || entity->x < -60
			|| entity->y > 200 || entity->y < -40) {
			entity->removed = true;
	}
}
if (entity->isVisible) {
...
void playerGun() {
	if (shotgunTime <= 0) {
		uProjectile *projectile = createProjectile(player.entity.x + player.entity.width, player.entity.y + (player.entity.height/2), BULLET_WIDTH, BULLET_HEIGHT, Bullet);
		projectile->damage = 50;
	} else {
		uProjectile *projectile = createProjectile(player.entity.x + player.entity.width, player.entity.y + (player.entity.height/2), BULLET_WIDTH, BULLET_HEIGHT, Bullet);
		projectile->damage = 50;
		uProjectile *projectileUp = createProjectile(player.entity.x + player.entity.width, player.entity.y + (player.entity.height/2), BULLET_WIDTH, BULLET_HEIGHT, Bullet);
		projectileUp->damage = 50;
		projectileUp->entity.yVel = -1;
		uProjectile *projectileDown = createProjectile(player.entity.x + player.entity.width, player.entity.y + (player.entity.height/2), BULLET_WIDTH, BULLET_HEIGHT, Bullet);
...
void playerRocket() {
	uProjectile *projectile = createProjectile(player.entity.x + player.entity.width, player.entity.y + (player.entity.height/2), ROCKET_WIDTH, ROCKET_HEIGHT, Rocket);
	projectile->entity.xVel = 3;
	projectile->damage = 300;
}

void shootBullet(uShip ship) {
	uProjectile *projectile = createProjectile(ship.entity.x -8, ship.entity.y + (ship.entity.height/2), REDBULLET_WIDTH, REDBULLET_HEIGHT, RedBullet);
	projectile->team = 1;
	projectile->entity.xVel = -4;
...
void ai5(uShip *ship) {
	...
	if (ship->reload <= 0) {
		uShip *missile =createEnemyShip(ship->entity.x - (ship->entity.width + 10), ship->entity.y + (ship->entity.height/2), MISSILE_WIDTH, MISSILE_HEIGHT, Missile);
		missile->health = 10;
		missile->ai = 4;
		ship->reload = 90;
	}
...
/*
void convertImages(vu16 toChange, vu16 toChangeTo) {
	convertImage(KusInt, toChange, toChangeTo);
}

void convertImage(const unsigned short *image, vu16 toChange, vu16 toChangeTo) {
